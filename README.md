...
模型调用默认每次尝试最多 10 分钟，包含重试与退避总共最多 20 分钟。启动前可通过环境变量或 `.env` 配置（单位秒，必须为有限正数）：

| 环境变量 | 默认值 | 范围 |
| --- | ---: | --- |
| `MECODE_REQUEST_TIMEOUT_SECONDS` | `600` | 一次 HTTP 请求，包含等待响应、流式读取及事件交付等待 |
| `MECODE_TOTAL_TIMEOUT_SECONDS` | `1200` | 一次模型调用的全部尝试、事件交付等待及退避，重试不重置 |

两种时限均不被心跳或思考数据刷新；原有 120 秒网络阶段超时仍保留。单次超时仅在尚未交付正文/完整工具时允许有限重试，总时限到期则停止本次调用。已显示正文保留，不自动从头重放。超时会取消请求并等待连接清理，因此返回错误可能略晚于时限；手动停止仍可用。下一次模型调用重新计时，工具执行及整个 Agent 多轮任务不共用这 20 分钟。

代码中可用 `Provider(backend, request_timeout=600, total_timeout=1200)` 显式覆盖环境配置。
