...
# 身份 + 工作方式 + 行为准则（静态部分）
_HEAD = """你是一个编程助手，可以用工具读、改、运行、搜索代码，帮用户完成开发任务。

工作方式："""

# 工作方式逐条 =（正文, 这条讲的工具）。工具被 MECODE_DISABLE_TOOLS 摘掉后，讲它用法的那条
# 也要跟着消失——否则提示词还在教模型用一个注册表里已经没有的工具，它会照着调、拿回
# "没有名为 X 的工具"，白费轮次。（实测：无外网环境关掉 web 工具，80 次跑批里模型仍尝试
# web_search 37 次，全部报错收场。）
# 空 tuple = 恒常在：这几条讲的是核心工具，禁用它们等于让 agent 没法干活，不做条件化。
_WORK: tuple[tuple[str, tuple[str, ...]], ...] = (
...
def base_prompt(disabled: Collection[str] = ()) -> str:
    """身份 + 工作方式 + 行为准则。

    disabled = 被 MECODE_DISABLE_TOOLS 摘掉的工具名；某条讲到的工具只要有一个被摘，
    这条就整条不发（"web_search 搜 → web_fetch 读全文"少了任何一半都不成立）。

    按【禁用名单】过滤而不是按【可用工具】过滤，有两个原因：
    ① 注册顺序：subagent / task_* 是 Agent.__init__ 里注册的，比本函数被调用的时机晚，
       按"当前注册表有什么"过滤会把这两条误删；
    ② 语义：disabled_tools() 只作用于 default_registry()，Agent 自注册的那批本来就禁不掉，
       "有没有"回答不了"该不该讲"，"是不是被显式摘掉了"才能。
    """
    off = set(disabled)
    keep = [line for line, gate in _WORK if not (off & set(gate))]
    return "\n".join([_HEAD, *keep, "", _TAIL])


BASE = base_prompt()      # 全量版：给不关心工具裁剪的调用方/测试用
...
def build_system_prompt(extra: str = "", project_context: str | None = None,
                        ...
                        disabled: Collection[str] | None = None) -> str:
    """拼出最终 system prompt：静态在前、环境放最后（利于 prompt 缓存）。

    project_context 默认（None）自动从项目根读 CLAUDE.md；显式传入则用传入的
    （传空串 "" 表示明确不要项目说明、也不自动加载，方便测试）。

    disabled 默认（None）自己读 MECODE_DISABLE_TOOLS，用来裁掉讲已禁用工具的那几条
    （见 base_prompt）。仍是【静态前缀】：同一份部署每次开会话都一样，不影响 prompt 缓存。
    ...
    parts = [base_prompt(disabled), SAFETY]
